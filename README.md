        ...
        async function sincronizarCloud() {
            ...
            if (!usuarioCloud) {
                ...
                try {
                    ...
                    // Se erro, tentar criar conta
                    if (error) {
                        const criar = confirm('Conta não encontrada. Criar nova?');
                        if (!criar) {
            ...
            // SINCRONIZAR
            const badge = document.getElementById('cloudBadge');
            const textoOriginal = badge.innerHTML;
            
            try {
                // Corrigir IDs decimais
                parcelas.forEach(p => {
                    if (!Number.isInteger(p.id)) p.id = Math.floor(p.id);
                });
                
                // UPLOAD EM BATCH (mais rápido)
                badge.innerHTML = '⬆️ Enviando...';
                ...
                if (compras.length > 0) {
                    ...
                    await supabaseClient.from('compras').upsert(comprasData);
                }
                ...
                if (vendas.length > 0) {
                    const vendasData = vendas.map(v => ({
                        ...
                        num_parcelas: v.numParcelas,
                        taxa_juros: 0,
                        status: v.status,
                        user_id: usuarioCloud.id
                    }));
                    
                    await supabaseClient.from('vendas').upsert(vendasData);
                }
                ...
                if (parcelas.length > 0) {
                    ...
                    await supabaseClient.from('parcelas').upsert(parcelasData);
                }
                
                // DOWNLOAD
                badge.innerHTML = '⬇️ Baixando...';
                
                const { data: comprasCloud } = await supabaseClient
                    .from('compras')
                    .select('*')
                    .eq('user_id', usuarioCloud.id);
                
                const { data: vendasCloud } = await supabaseClient
                    .from('vendas')
                    .select('*')
                    .eq('user_id', usuarioCloud.id);
                
                const { data: parcelasCloud } = await supabaseClient
                    .from('parcelas')
                    .select('*')
                    .eq('user_id', usuarioCloud.id);
